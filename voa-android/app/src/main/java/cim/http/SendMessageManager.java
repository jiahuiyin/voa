...
public class SendMessageManager{
    ...
    public static void send(final Message message,final String token) {

        MessageService messageService = createService(MessageService.class);

        messageService.send(message.getSender(), message.getReceiver(),message.getAction(),message.getContent(),token).enqueue(new Callback<Void>() {
            @Override
